<template>
    <!-- 导入表 -->
    <el-dialog title="导入表" v-model="visible" width="40%" top="5vh" append-to-body @close="celanParentTableSelect">
        <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="60px" label-position="left">
            <el-form-item label="表名称" prop="tableName">
                <el-input maxlength="100" v-model="queryParams.tableName" placeholder="请输入表名称" clearable style="width: 240px" />
            </el-form-item>
            <el-form-item label="表描述" prop="tableComment">
                <el-input maxlength="100" v-model="queryParams.tableComment" placeholder="请输入表描述" clearable style="width: 240px" />
            </el-form-item>
            <form-search @reset="resetQuery" @search="handleQuery" />
        </el-form>
        <el-row>
            <el-table stripe border height="520px" ref="tableRef" :data="dbTableList" @row-click="clickRow" @selection-change="handleSelectionChange">
        ...
        <template #footer style="text-align: center">
            <div class="dialog-footer">
                <el-button @click="visible = false">{{ $t('components.btn.cancelButton') }}</el-button>
                <!-- prettier-ignore -->
                <el-button type="primary" :disabled="okBtnEnabled" @click.native="handleImportTable()" >{{$t('components.btn.confirmButton')}}</el-button>
            </div>
        </template>
    </el-dialog>
</template>
