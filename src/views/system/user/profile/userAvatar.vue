<template>
    <div>
        <div class="user-info-head" @click="editCropper()">
            <img v-bind:src="options.img" title="" class="img-circle img-lg" />
        </div>
        <el-dialog :title="$t('navbar.personalInfo.userAvatar.title')" v-model="open" width="800px" append-to-body @opened="modalOpened">
            <el-row>
            ...
            <el-row>
                <el-col :lg="2" :md="2">
                    <el-upload action="#" :http-request="requestUpload" :show-file-list="false" :before-upload="beforeUpload">
                        <el-button size="small">
                            {{ $t('navbar.personalInfo.userAvatar.select') }}
                ...
                <el-col :lg="{ span: 2, offset: 6 }" :md="2">
                    <!-- prettier-ignore -->
                    <el-button type="primary" size="small" @click="uploadImg()">{{ $t('navbar.personalInfo.userAvatar.submit') }}</el-button>
                </el-col>
            </el-row>
        </el-dialog>
    </div>
...
const beforeUpload = (file: any) => {
    if (file.type.indexOf('image/') == -1) {
        // prettier-ignore
        proxy.$modal.msgError($t('components.unload.message'));
    } else {
    ...
    proxy.$refs.cropper.getCropBlob((data: any) => {
        ...
        uploadAvatar(formData).then((response: any) => {
            if (response.code === 200) {
                ...
                proxy.$modal.msgSuccess($t('components.message.upload'))
                visible.value = false
            }
        })
    })
