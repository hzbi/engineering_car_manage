...
export default () => {
    ...
    const { role_status } = proxy.useDict('role_status')
    // 数据范围选项
    const dataScopeOptions = [
        {
            value: '1',
            label: useAppStore().language == 'zh' ? '全部数据权限' : 'All data permissions'
        }
        // {
        //     value: '2',
        //     label: useAppStore().language == 'zh' ? '自定数据权限' : ''
        // },
        // {
        //     value: '3',
        //     label: useAppStore().language == 'zh' ? '本部门数据权限' : ''
        // },
        // {
        //     value: '4',
        //     label: useAppStore().language == 'zh' ? '本部门及以下数据权限' : ''
        // },
        // {
        //     value: '5',
        //     label: useAppStore().language == 'zh' ? '仅本人数据权限' : ''
        // }
    ]
    // 菜单列表
    ...
    const rules = {
        roleName: [
            {
                required: true,
                message: useAppStore().language == 'zh' ? '角色名称不能为空' : 'Character name cannot be empty',
                trigger: 'blur'
            }
        ],
        roleKey: [
            {
                required: true,
                message: useAppStore().language == 'zh' ? '权限字符不能为空' : 'Permission character cannot be empty',
                trigger: 'blur'
            }
        ],
        roleSort: [
            {
                required: true,
                message: useAppStore().language == 'zh' ? '角色顺序不能为空' : 'The role order cannot be empty.',
                trigger: 'blur'
            }
        ]
    }
    ...
    const handleStatusChange = async (val: any, row: any) => {
        ...
        let text = val == '0' ? (useAppStore().language == 'zh' ? '启用' : 'Enable') : useAppStore().language == 'zh' ? '停用' : 'Disable'
        // prettier-ignore
        await ElMessageBox.confirm(
            useAppStore().language == 'zh' ?`确认要${text}"${row.roleName}"角色吗?`:'', '警告', {
           confirmButtonText: $t('components.btn.confirmButton'),
                       cancelButtonText: $t('components.btn.cancelButton')
        })
            .then(() => {
                return changeRoleStatus(row.roleId, val)
            })
            .then((response: any) => {
                if (response.code === 200) {
    ...
    const handleUpdate = (row: any) => {
        ...
        getRole(roleId).then((response: any) => {
            if (response.code === 200) {
                ...
                open.value = true
                nextTick(() => {
                    getRoleMenuTreeselect(roleId).then((res: { code: number; data: { checkedKeys: any } }) => {
                        if (res.code === 200) {
    ...
    const handleDataScope = (row: any) => {
        ...
        getRole(row.roleId).then((response: any) => {
            if (response.code === 200) {
                ...
                setTimeout(() => {
                    nextTick(() => {
                        getRoleDeptTreeselect(row.roleId).then((res: { data: { checkedKeys: any } }) => {
                            // prettier-ignore
                            deptRef.value?.setCheckedKeys(res.data.checkedKeys);
                        })
                    })
                }, 100)
                title.value = $t('dialog.roleTitle')
            }
        })
    }
    /** 提交按钮 */
    ...
    /** 导出按钮操作 */
    const handleExport = () => {
        proxy.download('/system/role/exportByStream', { ...queryParams, ids: ids.value.join(',') }, `${$t('menu.Role')}${new Date().getTime()}.xlsx`)
    }
